...
public class SparkManager {
    ...
    // 火花半径
    public   float SPARK_RADIUS = 16.0F;

    // 火花外侧阴影大小
    private static final float BLUR_SIZE = 5.0F;

    // 每帧速度
    private static final float PER_SPEED_SEC = 3.0F;

    // 随机数
    private Random mRandom = new Random();

    // 火花的起始点，终点，贝赛尔曲线拐点1，塞贝儿曲线拐点2
    ...
    public int[] drawSpark(Canvas canvas, int x, int y, int[] store) {
        ...
        if (mCurDistance == mDistance && isActive) {
            mDistance = getRandom(100, mRandom.nextInt(15)) + 1;
            mCurDistance = 0;

            start = new Point(X, Y);
            end = getRandomPoint(start.x, start.y, (int) mDistance);
            c1 = getRandomPoint(start.x, start.y, mRandom.nextInt(100/ 16));
            c2 = getRandomPoint(end.x, end.y, mRandom.nextInt(100 / 16));
        }
        // 恢复火花路径
        else {
